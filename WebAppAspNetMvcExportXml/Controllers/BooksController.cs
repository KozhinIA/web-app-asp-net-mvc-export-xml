...
namespace WebAppAspNetMvcExportXml.Controllers
{
    public class BooksController : Controller
    {
        ...
        {
            var db = new LibraryContext();
            var books = db.Books.ToList().Select(x => new XmlBook()
            {
                ...
                Name = x.Name,
                Year = x.Year,
                Authors = x.Authors.Select(y => new XmlAuthor() { Id = y.Id }).ToList(),
                Languages = x.Languages.Select(y => new XmlLanguage() { Id = y.Id }).ToList(),
                BookImage = x.BookImage == null ? null : new XmlBookImage()
                {
                    ContentType = x.BookImage.ContentType,
                    FileName = x.BookImage.FileName,
                    Data = Convert.ToBase64String(x.BookImage.Data)
                }
            }).ToList();
